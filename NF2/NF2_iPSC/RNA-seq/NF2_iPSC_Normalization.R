...
cat("\014")
rm(list = ls())
setwd("C:/Workspace/R/NF2/NF2_iPSC/RNA-seq")
...
pca_plot <- ggplot(PCA_values, aes(PC1, PC2, fill = cell_line)) +
  ...
  labs(
    x = paste0("PC1: ",percentVar_iPSC[1],"% variance"),
    y = paste0("PC2: ",percentVar_iPSC[2],"% variance")) 
pcapl_iPSC
ggsave("Figure/PCA_NF2_iPSC.tiff", pcapl_iPSC, units = "in", width = 2.15, height = 2.15, device = "tiff", dpi = 300)
