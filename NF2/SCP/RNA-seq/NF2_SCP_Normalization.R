...
cat("\014")
rm(list = ls())
setwd("C:/Workspace/R/NF2/SCP/RNA-seq")
...
# Load count matrix -------------------------------------------------------
NF2_SCP_RC <- read.csv("Raw_data/NF2_SCP_RC.csv")
raw_data <- NF2_SCP_RC
...
ctrl_sample <- c("Wi", "Ctrl")
cell_group <- factor(rep(c("Ctrl", "Pat", "Pat_Dox", "KO", "KO_Dox", "Ctrl", "Pat"), each = 3)) %>% 
  relevel(ctrl_sample)
cell_line <- factor(rep(c("Wi", "SiN", "SD", "HiN", "HD", "F1", "O3"), each = 3)) %>% 
  relevel(ctrl_sample)
...
write.csv(annotation_vst, "Export/NF2_SCP_VST.csv")

dds_LRT <- DESeq(keep_ddsm, test = "LRT", reduced = ~1)
vst_LRT <- vst(dds_LRT, blind = F)
annotation_LRT <- as.data.frame(assay(vst_LRT)) %>% 
  mutate(hgnc_symbol = mapIds(org.Hs.eg.db, keys = row.names(vst_LRT), keytype = "ENSEMBL", column = "SYMBOL"))
write.csv(annotation_vst, "Export/NF2_SCP_VST_LRT.csv")

LRT_design <- resultsNames(dds_LRT)[-1]
for (i in LRT_design) {
  annotation_result <- as.data


experiment_df <- resultsNames(dds_LRT)[2:8]
result_dds <- results(experiment_df)
