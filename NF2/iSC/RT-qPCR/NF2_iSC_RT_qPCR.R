...
rm(list = ls())
cat("\014")
setwd("C:/Workspace/R/NF2/iSC/RT-qPCR")
...
sample = unique(summary_1$sample)
cell_line = rep(c("Wi", "F1", "SiN", "O3", "N50F"), each = 2)
cell_type =  c(rep("iSC", times = 8), rep("iFibro", times = 2))
genotype =  c(rep("Ctrl", times = 4), rep("Patient", times = 4), rep("Ctrl", times = 2))

sample_1 <- data.frame(sample, cell_line, cell_type, genotype)

stat_1 <- left_join(summary_1, sample_1, by = "sample") %>% 
  filter(sample %in% unique(summary_1$sample), target.x != "NF2")
